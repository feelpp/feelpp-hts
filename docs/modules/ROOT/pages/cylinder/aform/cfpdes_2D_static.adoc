...
:description: 2D magnetic static model using A Formulation and Erf function
...
NOTE: This function is not implemented on *feelpp*, so we use a *fit* on a .csv with a large panel of values calculated with the function.
...
Download export files to view results on https://www.paraview.org/download/[Paraview] : https://girder.math.unistra.fr/api/v1/folder/64ccc011b0e9570499e1d073/download[Cylinder_Aform_2D.zip]