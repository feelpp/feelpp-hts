...
:description: Magnetic transient model on a HTS bulk cylinder geometry surrounded by air in 2D coordinates using the H Formulation and the E-J power law
...
// We compare the current density profiles with *Feelpp* at stem:[t=7s] and *FreeFEM* on the stem:[O_r] axis, on the diameter of the cylinder, for a maximum applied field of 0.02 T.
// ++++
// <div id="myDiv"></div>
