...
Group {
    ...
    DefineConstant [testname = "tapes_2D_model"];
...
Function{
    ...
    // Excitation
    DefineConstant [IFraction = {1, Name "Input/4Source/0Fraction of max. current intensity (-)"}];
    DefineConstant [Imax = IFraction*jc*W_tape*H_tape]; // Maximum imposed current intensity [A]
    DefineConstant [I0 = 70]; // Initial imposed current intensity [A]
    ...
    // Control points
    controlPoint1 = {0,0.1e-3, 0}; // CP1
    controlPoint2 = {W_tape, 0.1e-3, 0}; // CP2
    controlPoint3 = {0.1e-3, 0, 0}; // CP3
    controlPoint4 = {0.1e-3, 14*0.21*1e-3, 0}; // CP4
...
Constraint {
    { Name a ;
        Case {
            {Region SurfOut ; Value -0.12*X[] ; TimeFunction hsVal[] ;}
        }
    }
    { Name phi ;
        Case {
    ...
    { Name Current ; Type Assign;
        Case {
            ...
            ElseIf(formulation == ta_formulation)
                // t-a-formulation
                { Region Edge1; Value 0.0; TimeFunction I[]; } // t_tilde = w t
            EndIf
        }
    }
    ...
    { Name MagDyn;LastTimeStepOnly realTimeSolution ;
        ...
        Operation {
            If(economPos == 0)
                ...
                EndIf
                Print[ norm_j, OnElementsOf OmegaC , File "res/j.pos", Name "j [A/m2]" ];
                ...
                Print[ norm_h, OnElementsOf Omega , File "res/h.pos", Name "h [A/m]" ];
                If(formulation == ta_formulation)
                    Print[ norm_b, OnElementsOf OmegaCC , File "res/b.pos", Name "b [T]" ];
                Else
