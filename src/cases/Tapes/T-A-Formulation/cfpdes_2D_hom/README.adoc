...
[source,sh]
----
mpirun -np 16 feelpp_toolbox_coefficientformpdes --config-file=tapes_2D.cfg
----
...
stem:[\rho=\frac{E_c}{J_c}\left(\frac{\mid\mid J \mid\mid}{J_c}\right)^{(n)}=\frac{E_c}{J_c}\left(\frac{\mid\mid \partial_x T_r \mid\mid}{J_c}\right)^{(n)}]
...
|image::Tapes/T-A-Formulation/cfpdes_2D/geozoom.png[,width=600]
|===
...
The case data files are available in Github link:{uri-dataT-}/Tapes/T-A-Formulation/cfpdes_2D_hom[here]
